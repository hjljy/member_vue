        ...
        <el-form ref="addForm" class="addForm" :rules="rules" :model="sysOrg" label-width="80px">
          ...
          <el-form-item label="所属机构" prop="orgId">
            <el-select v-model="sysOrg.pid" placeholder="机构" :disabled="noSelfChange">
              <el-option v-for="(item, id) in roles" :key="id" :label="item.id" :value="item">{{ item.name }}</el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="角色权限" prop="roleIds">
            <el-checkbox-group v-model="sysOrg.roleIds" :disabled="noSelfChange">
          ...
          <el-form-item label="机构描述" prop="description">
            <el-input v-model="sysOrg.description" type="textarea" :disabled="noShow" placeholder="这个人很神秘，什么都没有留下" />
          </el-form-item>
