<template>
  <div style="padding: 30px">
    ...
    <el-form :inline="true" :model="queryParam" class="form-inline">
      ...
      <el-form-item>
        <el-button v-permission="['sys:user:add']" type="primary" @click="handleEdit(true)">
          新增
        </el-button>
      </el-form-item>
    ...
    <el-drawer :title="title" :visible.sync="dialog" :show-close="false" size="20%" :closed="handleClose('addForm')">
      <div>
        <el-form ref="addForm" class="addForm" :rules="rules" :model="sysUser" label-width="80px">
          <el-form-item label="用户账号" prop="userName">
            <el-input v-model="sysUser.userName" placeholder="长度在12个字符以内,不能是纯数字" :disabled="noChange" />
          </el-form-item>
          ...
          <el-form-item label="所属机构" :disabled="noChange">
            <el-select v-model="sysUser.orgId" placeholder="机构" :disabled="noChange">
              <el-option label="默认" value="-1" />
            </el-select>
          </el-form-item>
          <el-form-item label="角色权限" prop="roles">
            <el-checkbox-group v-model="sysUser.roles" :disabled="noChange">
  ...
  methods: {
    ...
    handleEdit(isAdd, row) {
      if (isAdd) {
        ...
        this.title = '编辑用户'
        this.sysUser = row
        this.sysUser.roles = row.roles.map(item => item.id)
      }
    }
  }
}
