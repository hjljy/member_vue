  ...
  <div style="padding: 20px">
    ...
    <el-form :inline="true" class="form-inline">
      <el-form-item style="float:right">
        <el-button v-permission="['member:level:add']" style="float:right" type="primary" @click="openForm('add')">新增</el-button>
      </el-form-item>
    ...
    <el-dialog title="会员等级" :visible.sync="dialogFormVisible">
      <el-form ref="addForm" :model="memberLevel" :rules="rules" :inline="true">
        ...
        <el-form-item label="所需成长值:" :label-width="formLabelWidth" prop="upgradeGrowthValue">
          <el-input v-model.number="memberLevel.upgradeGrowthValue" placeholder="请输入成长值" :disabled="nochange" autocomplete="off" />
        </el-form-item>
      </el-form>
...
<script>
import { getMemberLevelList, addMemberLevel, editMemberLevel, delMemberLevel } from '@/api/member/memberLevel'
...
export default {
  ...
  data() {
    var checkUpv = (rule, value, callback) => {
      if (this.memberLevel.levelId) {
        if (this.memberLevel.levelOrder !== 1) {
          var minOrder = this.memberLevel.levelOrder - 1
          var minValue = 0
          var maxOrder = this.memberLevel.levelOrder + 1
          var maxValue = 1000000000
          this.tableData.forEach(n => {
            if (n.levelOrder === minOrder) {
              minValue = n.upgradeGrowthValue
            }
            if (n.levelOrder === maxOrder) {
              maxValue = n.upgradeGrowthValue
            }
          })
          if (value <= minValue || value >= maxValue) {
            callback(new Error('所需成长值范围在' + minValue + '-' + maxValue))
          } else {
            callback()
          }
        }
        ...
        if (value < 0) {
          callback(new Error('所需成长值需要大于等于0'))
        }
  ...
  methods: {
    ...
    handleAdd(formName) {
      ...
      if (this.memberLevel.levelId) {
        this.$refs[formName].validate((valid, message) => {
          if (valid) {
            editMemberLevel(this.memberLevel).then(response => {
    ...
    openForm(type, data) {
      ...
      // 如果是新增
      if (type === 'add') {
        if (this.maxLevelOrder === 0) {
          this.memberLevel.memberDefault = 'true'
          this.nochange = true
        } else {
          this.memberLevel.upgradeGrowthValue = this.maxUpgradeGrowthValue * 2
        }
      } else if (type === 'edit') {
        if (data) {
          this.memberLevel.levelId = data.levelId
          this.memberLevel.orgId = data.orgId
          this.memberLevel.levelName = data.levelName
          this.memberLevel.levelOrder = data.levelOrder
          this.memberLevel.upgradeGrowthValue = data.upgradeGrowthValue
          this.memberLevel.memberDefault = data.memberDefault.toString()
          if (data.levelOrder === 1) {
            this.nochange = true
          }
        }
      }
